...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
  ...
  if ($pickupTime !== 'ASAP') {
    try {
      $slotStmt = $db->prepare("SELECT COUNT(*) FROM orders WHERE pickup_time = ? AND status IN ('pending','preparing','ready') AND faculty_id != ?");
  ...
  if ($ok) {
    ...
    try {
      ...
      $db->prepare("INSERT INTO orders (order_number,order_type,status,faculty_id,total_amount,notes,pickup_time) VALUES (?,?,?,?,?,?,?)")
...
<script>
  ...
  document.getElementById('checkout-form').addEventListener('submit', function() {
    ...
    document.getElementById('cart-json').value = JSON.stringify(cartArr);
    sessionStorage.removeItem('faculty_cart'); // clear immediately before page unloads
  });
